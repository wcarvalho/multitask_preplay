...
def simulate_n_trajectories(
  ...
  # [N, ...]
  init_preds_t = jax.vmap(initial_predictions, in_axes=(None, None, 0), out_axes=(0))(
    x_t,
    h_tm1,
    jax.random.split(rng_, num_simulations),  # [D]  # [D]
  )
  init_a_t = policy_fn(init_preds_t, rng_)
  ...
  def _single_model_step(carry, inputs):
    ...
    ###########################
    rng, rng_ = jax.random.split(rng)
    next_preds, _ = network.apply(params, s, a, rng_, method=network.apply_model)
  ...
  return SimulationOutput(
    predictions=concat_first_rest(init_preds_t, sim_outputs.predictions),
...
class OfftaskDyna(vbb.RecurrentLossFn):
  ...
  temp_dist: distrax.Distribution = distrax.Gamma(concentration=1, rate=0.5)
  ...
  def dyna_loss_fn(
    ...
    def repeat_num_sim(x):
      return jax.vmap(identity, (None, 0), 0)(x, jnp.arange(self.num_simulations))
    ...
    # [D]
    first_preds_target, _ = self.network.apply(target_params, h_tm1_target, x_t, rng_)
...
class DynaAgentEnvModel(nn.Module):
  ...
  def __call__(
    ...
    predictions = Predictions(
      q_vals=q_vals, state=AgentState(timestep=x, rnn_state=new_rnn_state)
    )
  ...
  def unroll(
    ...
    predictions = Predictions(
      q_vals=q_vals, state=AgentState(timestep=xs, rnn_state=new_rnn_states)
    )
    return predictions, new_rnn_state
  ...
  def unroll_model(
    ...
    def body_fn(model, carry, inputs):
      ...
      predictions, _ = model.apply_model(state, action, rng__)
