"""
Craftax AI main results.

Migrated from figures/craftax_AI_results.py.

Usage:
    python plots/craftax_ai_results.py
    python plots/craftax_ai_results.py --refresh
"""
...
def get_metric_data_by_group(model_to_group=None, overwrite=False, debug=False):
  ...
  Args:
      model_to_group (dict, optional): Dictionary mapping model names to group names.
          Format: {
              'model_key': 'group_name',
              ...
          }
      overwrite (bool): Whether to overwrite cached data.
      debug (bool): Whether to run in debug mode (limited data).
...
def plot_achievement_bars(
  ...
  for idx, metric in enumerate(sorted_metrics):
    ...
    preplay_low = preplay_mean - preplay_sem  # noqa: F841
    preplay_high = preplay_mean + preplay_sem  # noqa: F841
    dyna_low = dyna_mean - dyna_sem  # noqa: F841
    dyna_high = dyna_mean + dyna_sem  # noqa: F841
...
def main():
  ...
  with open(os.path.join(OUTPUT_DIR, "seed_counts.txt"), "w") as f:
  ...
  print(f"Saved seed counts to {OUTPUT_DIR}/seed_counts.txt")
  ...
  save_figure(fig, "craftax_ai_results_achievement_bars_all")
  ...
  fig, axes = plot_baseline_craftax_comparison(
    ...
    save_path=os.path.join(OUTPUT_DIR, "craftax_ai_results_baseline_comparison.pdf"),
  )
