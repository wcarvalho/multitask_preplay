...
Migrated from random/top_rt_analysis.py.

Usage:
    python plots/jaxmaze_top_rt_analysis.py
"""
...
def main():
  ...
  fig.tight_layout()
  fig.savefig(
    os.path.join(OUTPUT_DIR, "jaxmaze_top_rt_analysis_difference_all_conditions.pdf"),
    bbox_inches="tight",
    dpi=300,
  )
  print(f"Saved to {OUTPUT_DIR}/top10_difference_all_conditions.pdf")
  plt.close(fig)
  ...
  for name, (means, sems) in difference_results.items():
    ...
    fig.savefig(
      os.path.join(OUTPUT_DIR, f"jaxmaze_top_rt_analysis_difference_{safe_name}.pdf"),
      bbox_inches="tight",
      dpi=300,
    )
    plt.close(fig)
  ...
  for ax, (name, (means, sems)) in zip(axes, relative_results.items()):
    ax.bar(
      rel_ranks, means, yerr=sems, capsize=3, color="darkorange", edgecolor="white"
    )
    ax.axhline(0, color="black", linewidth=0.8, linestyle="--")
    ax.set_title(name, fontsize=10)
  ...
  fig.tight_layout()
  fig.savefig(
    os.path.join(OUTPUT_DIR, "jaxmaze_top_rt_analysis_relative_all_conditions.pdf"),
    bbox_inches="tight",
    dpi=300,
  )
  print(f"Saved to {OUTPUT_DIR}/top10_relative_all_conditions.pdf")
  plt.close(fig)
  ...
  for name, (means, sems) in relative_results.items():
    ...
    safe_name = name.replace("\n", "_").replace(" ", "_").replace("+", "")
    fig.savefig(
      os.path.join(OUTPUT_DIR, f"jaxmaze_top_rt_analysis_relative_{safe_name}.pdf"),
      bbox_inches="tight",
      dpi=300,
    )
    plt.close(fig)
  ...
  fig.savefig(
    os.path.join(
      OUTPUT_DIR, "jaxmaze_top_rt_analysis_max_rt_position_all_conditions.pdf"
    ),
    bbox_inches="tight",
    dpi=300,
  )
  print(f"Saved to {OUTPUT_DIR}/top10_max_rt_position_all_conditions.pdf")
  plt.close(fig)
  ...
  for name, (means, sems) in position_results.items():
    ...
    fig.savefig(
      os.path.join(
        OUTPUT_DIR, f"jaxmaze_top_rt_analysis_max_rt_position_{safe_name}.pdf"
      ),
      bbox_inches="tight",
      dpi=300,
    )
