...
Migrated from random/overlap_distribution.py.

Usage:
    python plots/overlap_distribution.py
"""
...
from analysis import jaxmaze_analysis, vis_utils
...
def show_NaN_overlap_values(eval_df, full_df, save_dir):
  ...
  if len(nan_df) == 0:
    print("  No NaN overlap values found.")
    return
  ...
  for i in range(n_examples):
    ...
    for row_idx in range(n_rows):
      ...
      if row_idx < n_train:
        ...
        success_str = "Y" if train_row.get("success") else "N"
    ...
    save_figure(fig, f"nan_overlap_example_{i}", directory=save_dir)
    plt.close(fig)
...
def main():
  ...
  # Use the same filtered cohorts as the main analysis
  ...
  nan_diag_dir = os.path.join(OUTPUT_DIR, "overlap_nan_diagnostics")
  show_NaN_overlap_values(paths_df, paths_all_df, os.path.join(nan_diag_dir, "paths"))
  show_NaN_overlap_values(
    shortcut_df, shortcut_all_df, os.path.join(nan_diag_dir, "shortcut")
  )
  ...
  # Save figure
  save_figure(fig, "overlap_distribution", directory=OUTPUT_DIR)
