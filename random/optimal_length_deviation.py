...
SAVE_DIR = "random/plots/optimal_length"


# ---------------------------------------------------------------------------
# JaxMaze helpers (unchanged logic, uses unified get_experiment_data)
# ---------------------------------------------------------------------------
...
def visualize_anomalous_episodes(anomalous_df, experiment):
  ...
  out_dir = f"{SAVE_DIR}/{experiment}/anomalous"
  ...
  for i, row in enumerate(anomalous_df.iter_rows(named=True)):
    ...
    save_path = f"{out_dir}/anomalous_{i:03d}_user={user_id}.png"
...
def save_example_train_paths(human_train, model_train, experiment):
  ...
  out_dir = f"{SAVE_DIR}/{experiment}/example_train_paths"
  os.makedirs(out_dir, exist_ok=True)
  ...
  if len(human_train) > 0:
    ...
    fig, _ = visualize_jaxmaze_row(row, title=title)
    fig.savefig(f"{out_dir}/human.png", bbox_inches="tight", dpi=150)
    plt.close(fig)
    print(f"  Saved human example to {out_dir}/human.png")
  ...
  for algo in model_train["algo"].unique().sort().to_list():
    ...
    fig, _ = visualize_jaxmaze_row(row, title=title)
    fig.savefig(f"{out_dir}/{algo}.png", bbox_inches="tight", dpi=150)
    plt.close(fig)
    print(f"  Saved {algo} example to {out_dir}/{algo}.png")
...
def _save_bar_plot(train_stats, test_stats, experiment_label):
  ...
  save_path = f"{SAVE_DIR}/bar_plot_{experiment_label}.png"
